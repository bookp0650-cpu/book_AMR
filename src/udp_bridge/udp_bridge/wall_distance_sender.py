...
class WallDistanceSender(Node):
    def __init__(self):
        # ⚠️ YAMLと一致
        super().__init__('wall_distance_sender')

        # -------------------------
        # 🔹 パラメータ宣言
        # -------------------------
        self.declare_parameter('target_ip', '172.20.10.3')
        self.declare_parameter('tcp_port', 5025)
        self.declare_parameter('retry_interval', 1.0)
        self.declare_parameter('topic_name', '/wall_distance')
